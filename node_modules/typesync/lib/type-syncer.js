"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var util_1 = require("./util");
/**
 * Creates a type syncer.
 *
 * @param packageJSONservice
 * @param typeDefinitionSource
 */
function createTypeSyncer(packageJSONService, typeDefinitionSource, globber) {
    var getLatestTypingsVersion = util_1.memoizeAsync(typeDefinitionSource.getLatestTypingsVersion);
    return {
        sync: sync
    };
    /**
     * Syncs typings in the specified package.json.
     */
    function sync(filePath, opts) {
        if (opts === void 0) { opts = { dry: false }; }
        return __awaiter(this, void 0, void 0, function () {
            var _a, file, allTypings, subPackages, syncedFiles;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0: return [4 /*yield*/, Promise.all([
                            packageJSONService.readPackageFile(filePath),
                            typeDefinitionSource.fetch()
                        ])];
                    case 1:
                        _a = _b.sent(), file = _a[0], allTypings = _a[1];
                        return [4 /*yield*/, Promise.all(util_1.ensureWorkspacesArray(file.packages).concat(util_1.ensureWorkspacesArray(file.workspaces)).map(globber.globPackageFiles))
                                .then(util_1.flatten)
                                .then(util_1.uniq)];
                    case 2:
                        subPackages = _b.sent();
                        return [4 /*yield*/, Promise.all([
                                syncFile(filePath, file, allTypings, opts)
                            ].concat(subPackages.map(function (p) { return syncFile(p, null, allTypings, opts); })))];
                    case 3:
                        syncedFiles = _b.sent();
                        return [2 /*return*/, {
                                syncedFiles: syncedFiles
                            }];
                }
            });
        });
    }
    /**
     * Syncs a single file.
     *
     * @param filePath
     * @param file
     * @param allTypings
     * @param opts
     */
    function syncFile(filePath, file, allTypings, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var _a, allPackages, allPackageNames, newTypings, devDepsToAdd;
            var _this = this;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _a = file;
                        if (_a) return [3 /*break*/, 2];
                        return [4 /*yield*/, packageJSONService.readPackageFile(filePath)];
                    case 1:
                        _a = (_b.sent());
                        _b.label = 2;
                    case 2:
                        file = _a;
                        allPackages = getPackagesFromSection(file.dependencies).concat(getPackagesFromSection(file.devDependencies), getPackagesFromSection(file.optionalDependencies), getPackagesFromSection(file.peerDependencies));
                        allPackageNames = util_1.uniq(allPackages.map(function (p) { return p.name; }));
                        newTypings = filterNewTypings(allPackageNames, allTypings);
                        return [4 /*yield*/, Promise.all(newTypings.map(function (t) { return __awaiter(_this, void 0, void 0, function () {
                                var _a, latestVersion, codePackage, semverRangeSpecifier;
                                return __generator(this, function (_b) {
                                    switch (_b.label) {
                                        case 0: return [4 /*yield*/, getLatestTypingsVersion(t.typingsName)];
                                        case 1:
                                            latestVersion = _b.sent();
                                            codePackage = allPackages.find(function (p) { return p.name === t.codePackageName; });
                                            semverRangeSpecifier = codePackage
                                                ? getSemverRangeSpecifier(codePackage.version)
                                                : '^';
                                            return [2 /*return*/, (_a = {},
                                                    _a[util_1.typed(t.typingsName)] = semverRangeSpecifier + latestVersion,
                                                    _a)];
                                    }
                                });
                            }); })).then(util_1.mergeObjects)];
                    case 3:
                        devDepsToAdd = _b.sent();
                        if (!!opts.dry) return [3 /*break*/, 5];
                        return [4 /*yield*/, packageJSONService.writePackageFile(filePath, __assign({}, file, { devDependencies: util_1.orderObject(__assign({}, devDepsToAdd, file.devDependencies)) }))];
                    case 4:
                        _b.sent();
                        _b.label = 5;
                    case 5: return [2 /*return*/, {
                            filePath: filePath,
                            newTypings: newTypings,
                            package: file
                        }];
                }
            });
        });
    }
}
exports.createTypeSyncer = createTypeSyncer;
/**
 * Returns an array of new typings as well as the code package name that was matched to it.
 *
 * @param allPackageNames Used to filter the typings that are new.
 * @param allTypings All typings available
 */
function filterNewTypings(allPackageNames, allTypings) {
    var existingTypings = allPackageNames.filter(function (x) { return x.startsWith('@types/'); });
    return util_1.filterMap(allPackageNames, function (p) {
        var scopeInfo = getPackageScope(p);
        if (scopeInfo && scopeInfo[0] !== 'types') {
            p = scopeInfo[0] + "__" + scopeInfo[1];
        }
        var typingsForPackage = allTypings.find(function (x) { return x.typingsName === p; });
        if (!typingsForPackage) {
            // No typings available.
            return false;
        }
        var fullTypingsPackage = util_1.typed(typingsForPackage.typingsName);
        var alreadyHasTyping = existingTypings.some(function (t) { return t === fullTypingsPackage; });
        if (alreadyHasTyping) {
            return false;
        }
        return __assign({}, typingsForPackage, { codePackageName: p });
    });
}
/**
 * If a package is scoped, returns the scope + package as a tuple, otherwise null.
 *
 * @param packageName Package name to check scope for.
 */
function getPackageScope(packageName) {
    var EXPR = /^\@([^\/]+)\/(.*)$/i;
    var matches = EXPR.exec(packageName);
    if (!matches) {
        return null;
    }
    return [matches[1], matches[2]];
}
/**
 * Gets packages from a dependencies section.
 *
 * @param section
 */
function getPackagesFromSection(section) {
    /* istanbul ignore next */
    if (!section) {
        return [];
    }
    var result = [];
    for (var name in section) {
        result.push({
            name: name,
            version: section[name]
        });
    }
    return result;
}
var CARET = '^'.charCodeAt(0);
var TILDE = '~'.charCodeAt(0);
/**
 * Gets the semver range specifier (~, ^)
 * @param version
 */
function getSemverRangeSpecifier(version) {
    if (version.charCodeAt(0) === CARET) {
        return '^';
    }
    if (version.charCodeAt(0) === TILDE) {
        return '~';
    }
    return '';
}
//# sourceMappingURL=type-syncer.js.map