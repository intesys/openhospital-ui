...
function run(syncer) {
    return __awaiter(this, void 0, void 0, function () {
        var _a, args, flags, _b, filePath, result, syncedFilesOutput, totalNewTypings;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    return [4 /*yield*/, C.spinWhile("Syncing type definitions in " + chalk_1.default.cyan(filePath) + "...", function () { return syncer.sync(filePath, { dry: flags.dry }); })];
                case 1:
                    result = _c.sent();
                    syncedFilesOutput = result.syncedFiles
                        .map(renderSyncedFile)
                        .join('\n\n');
                    totalNewTypings = result.syncedFiles
                        .map(function (f) { return f.newTypings.length; })
                        .reduce(function (accum, next) { return accum + next; }, 0);
                    C.success(totalNewTypings === 0
                        ? "No new typings added, looks like you're all synced up!"
                        : chalk_1.default(templateObject_2 || (templateObject_2 = __makeTemplateObject(["", " new typings added.\n\n", "\n\n\u2728  Go ahead and run {green npm install} or {green yarn} to install the packages that were added."], ["", " new typings added.\\n\\n", "\\n\\n\u2728  Go ahead and run {green npm install} or {green yarn} to install the packages that were added."])), totalNewTypings.toString(), syncedFilesOutput));
                    return [2 /*return*/];
            }
        });
    });
...
function renderTypeDef(typeDef, isLast) {
    var treeNode = isLast ? '└─' : '├─';
    return chalk_1.default(templateObject_3 || (templateObject_3 = __makeTemplateObject(["", " {gray @types/}{bold.blue ", "}"], ["", " {gray @types/}{bold.blue ", "}"])), treeNode, typeDef.typingsName);
}
/**
...
function renderSyncedFile(file) {
    var badge = file.newTypings.length === 0
        ? chalk_1.default(templateObject_4 || (templateObject_4 = __makeTemplateObject(["{blue.bold (no new typings added)}"], ["{blue.bold (no new typings added)}"]))) : chalk_1.default(templateObject_5 || (templateObject_5 = __makeTemplateObject(["{green.bold (", " new typings added)}"], ["{green.bold (", " new typings added)}"])), file.newTypings.length.toString());
    var title = chalk_1.default(templateObject_6 || (templateObject_6 = __makeTemplateObject(["\uD83D\uDCE6 ", " {gray.italic \u2014 ", "} ", ""], ["\uD83D\uDCE6 ", " {gray.italic \u2014 ",
        "} ", ""])), file.package.name, file.filePath, badge);
    var nl = '\n';
    return (title +
        nl +
        file.newTypings
            .map(function (t) {
            return renderTypeDef(t, file.newTypings[file.newTypings.length - 1] === t);
        })
            .join(nl));
}
/**
